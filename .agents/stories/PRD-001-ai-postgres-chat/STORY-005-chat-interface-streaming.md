...
- [ ] Given a message is sent, when the AI starts responding, then the tokens are streamed to the UI in real-time.
- [ ] Given the chat interface, when multiple messages are exchanged, then the history is displayed correctly.
- [ ] Given a database result set, when returned by the agent, then it is rendered as a clean table in the chat flow.
